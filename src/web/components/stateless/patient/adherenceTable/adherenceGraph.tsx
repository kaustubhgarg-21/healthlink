...
export const AdherenceGraph = (props: any) => {
  const {legendShift}=props;
  const {adhrence} = useSelector(PatientStateSelector);
  ...
  if(adhrence?.length > 0){
    let formatted:any= [];
    adhrence?.map((element:any)=>{
      ...
      if(exist >= 0) {
        ...
        updatedEl[key] = Math.round(element?.adherencePerc * 10)/10 || 0
        formatted.splice(exist,1, updatedEl)
      } else {
        const obj:any = {};
        const key:string = element?.biometricName
        obj[key] = Math.round(element?.adherencePerc * 10)/10 || 0
